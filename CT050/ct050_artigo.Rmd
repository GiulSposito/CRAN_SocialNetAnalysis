...
Vamos analisar frequencia de publicação por autores, excluindo autores que contribuíram apenas uma vez.
...
Observa-se que mais de 75% (3rd Quarter) dos autores contribuiram apenas 1 vez com outros autores, para estabelecer uma rede de colaboração mais efetiva (e não apenas momentânea), vamos filtrar na rede autores que  pelo menos 5 pacotes.
...
total_clusers <- nrow(clusters)

knitr::kable( head(clusters, 10), caption = "Comunidades de Autores dentro da Rede" )
  
```
Encontramos `r total_clusters` comunidades dentro da rede, dos quais duas se destacam pelo número de colaboradores bem acima 

## Conclusão

Vimos que as técnicas de 
